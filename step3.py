...
def run(dV):
    pdf = PdfPages('figures.{}.{}.step2.{}mV.pdf'.format(channel_name, prefix_fitting_output, dV))
    with open('{}.step1.{}mV.pk'.format(prefix, dV), 'rb') as fin:
        m = pickle.load(fin)
    ...
    with open('{}.{}.step2.{}mV.pk'.format(prefix, prefix_fitting_output, dV), 'wb') as fout:
        pickle.dump(m, fout)
    pdf.close()


# Fit all models to the same target states and estimate Q
channel = flowmsm.channel.Channel({dV:'{}.step1.{}mV.pk'.format(prefix, dV) for dV in dVs}, channel_name, dVs_fitting = dVs_fitting, use_md_data = False)
